...
_NOTE: The '-p' flag stands for 'prefix'.  These flags cannot be used together.  In order for a Conda environment with unique path or prefix to receive a name, either the 'CONDA\_ENVS\_PATH' environment variable must contain the environment within a subdirectory or the .condarc file at the top-level of that path must include the following entry._
...
_NOTE: This command will only work if the environment to be updated has already been ativated.  In order to update an evironment that is not currently active, include either the name or prefix flags as well._
...
The scripts are categorized into Ash and Bash directories for scripts compatible with the corresponding shells. Ash support is explicitly included to support Alpine-Linux-based containers out-of-the-box, while Bash support is provided as the more-powerful user-friendly alternative.
...
| Parameter  | Arguments | Definition                                                       |
| :--------- | :-------- | :--------------------------------------------------------------- |
